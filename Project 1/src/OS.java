...
public class OS {
	...
	/*public void runCommand() {
		...
		//	while (pcb.getProgramCounter() <= process.getCommands().size()) {
				for (int i = pcb.getProgramCounter(); i <= commands.size(); i++) {
					
					System.out.println("Running " + commands.get(i - 1).command);
					System.out.println("On Command: " + pcb.getProgramCounter() + "/" + commands.size());
					//scheduler.startQuantumClock();
					while (scheduler.quantumStatus) {

						// getPCB(process.getPID()).getProgramCounter() - 1;
						// If the process runs all of the cyles in the command, it will increament the
						// program counter
						// and exit out of the loop
						if (pcb.programCounter.getCyclesRan() == commands.get(i - 1).cycle) {
							pcb.programCounter.incrementProgramCounter();
							pcb.programCounter.setCyclesRan(0);
							//i = pcb.programCounter.getCounter();
							break;
						}

						pcb.programCounter.incrementProgramCycle();
						System.out.println(
								"On " + pcb.programCounter.getCyclesRan() + "/" + commands.get(i - 1).cycle + " cycle");

					}
					scheduler.quantumStatus = true;
					...
					if (pcb.programCounter.getCounter() > commands.size()) {
					...
					} else if (pcb.programCounter.getCyclesRan() < commands.get(i - 1).cycle
						...
						// in the dispatcher class
							...
							&& commands.get(i - 1).command.equals("CALCULATE")) {
